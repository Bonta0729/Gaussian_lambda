...
ガウス関数のような自然できれいな曲線になるのかも不明なので。仮にやるとしたらlambda＝(1-進行度)とかになる？でもそれだと進行度0の時はlambdaが1になってしまって大きすぎるから設定が難しい気がする。
...
実際はどうなのかは分かりません。  
一応参考までに手元の実験結果ではlambda_limit=256でlambdaをかなり大きくして(lambda0.9以上、lambda2を0.52～0.54位にして)良い結果が出ています。(特に定跡無しの平手開始局面で強いです。)  
...
(現状FV_SCALE＝24、ポナンザ定数360、Gaussian_lambdaのσ＝600、lambda_limit＝140で実験を繰り返しています。)
